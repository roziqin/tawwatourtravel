	...
	*/
	...
	if( !class_exists('tourmaster_pb_element_tour_category') ){
		class tourmaster_pb_element_tour_category{
			...
			static function get_content( $settings = array() ){
				...
				if( !empty($categories) && !is_wp_error($categories) ){
					if( empty($settings['style']) || $settings['style'] == 'grid' ){
						...
						$ret .= self::get_category_grid($categories, $settings);

					}else if( $settings['style'] == 'grid-2' ){

						$ret .= self::get_category_grid2($categories, $settings);

					}else if( $settings['style'] == 'widget' ){

						$ret .= self::get_category_widget($categories, $settings);
			...
			static function get_category_grid( $categories, $settings ){
				...
				foreach( $categories as $category ){
					...
					$ret .= '<a class="tourmaster-tour-category-head-link" href="' . esc_url(get_term_link($category->term_id, 'tour_category')) . '" >';
			...
			static function get_category_grid2( $categories, $settings ){
			...
			static function get_category_widget( $categories, $settings ){
